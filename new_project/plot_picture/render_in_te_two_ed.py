...
path_to_checkpoint = "C:\\Users\\HW\\ray_results\\first_text\\QMIX_grouped_test_dac76_00000_0_2023-04-14_12-50-18\\checkpoint_010000"
...
obss,_ = my_env.reset()
xx = obss["group_1"]
obs = np.array([j for i in range(2) for value in xx[i].values() for j in value]).reshape(1,46)
input_dict = {
    "obs": np.array([j for i in range(2) for value in xx[i].values() for j in value]).reshape(1,46),
...
while input_dict["terminateds"]==0 and input_dict["truncateds"]==0:
    ...
    obs = []
    obs = obss["group_1"]
    input_dict["obs"] = np.array([j for i in range(2) for value in obs[i].values() for j in value]).reshape(1,46)
