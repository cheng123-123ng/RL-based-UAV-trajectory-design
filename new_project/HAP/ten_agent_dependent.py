...
env1 = enviroment()
state_dim = (user_num+1,2)
action_dim = (user_num,1)
hidden_dim = 256
...
def two_agent_real_dependent(max_frames=12000):
#    max_frames  = 12000
    max_steps   = 10
    frame_idx   = 0
    rewards     = []
    ...
    #        ou_noise.reset()
        ...
        for step in range(max_steps):
            action1 = policy_net.get_action(state1.double())
    #            action = ou_noise.get_action(action, step)
            next_state1, reward1, done1 = env1.step(action1)
            if done1:
                replay_buffer_2.push(state1, action1, reward1, next_state1, done1)
            else:
                action2 = policy_net.get_action(next_state1.double())
                next_state2, reward2, done2 = env1.step(action2)
                if done2:
                    replay_buffer_2.push(next_state1, action2, reward2, next_state2, done2)
            replay_buffer_1.push(state1, action1, reward1, next_state1, done1)
            replay_buffer_1.push(next_state1, action2, reward2, next_state2, done2)
            ...
            if done1 or done2:
                break
#        if frame_idx % 500 == 0 and frame_idx>10:
