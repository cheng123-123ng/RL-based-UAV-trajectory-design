...
class MA_UavEnv0(MultiAgentEnv):
 ...
 #       super.__init__()
        ...
        self.ed_weight = config.get("ed_weight")
        self.nomal_obs = False if config==None else config.get("normal_obs")
        self.test = "normal" if config==None else config.get("test")
        ...
        # )
        #
        if self.nomal_obs ==True:
            self.observation_space = Dict(
                {
                    "action_mask": Box(0.0, 1.0, shape=(self.action_space.n,), dtype=int),
                    # "obs": Box(low=0, high=VIEWPORT_X, shape=((self.MAX_Uav_num* 3 + self.MAX_Ucar_num * 3 +self.ed_num*2,)), dtype=int),
                    "obs": Box(low=-1, high=1,
                               shape=((self.MAX_Uav_num * 3 + self.MAX_Ucar_num * 3 + self.ed_num * 2,)), dtype=int),
            ...
            self.observation_space = Dict(
                {
                    "action_mask": Box(0.0, 1.0, shape=(self.action_space.n,), dtype=int),
                    "obs": Box(low=-5, high=VIEWPORT_X, shape=((self.MAX_Uav_num* 3 + self.MAX_Ucar_num * 3 +self.ed_num*2,)), dtype=int),
                }
            )
        self.Uav = None
        self.Ucar = []
    ...
    def reset(self, *, seed=None, options=None):
        ...
        self.obs[0] = np.append(mid, values=np.array(d).reshape(self.ed_num*2, ), axis=0)

        mid = np.concatenate((b, a, c)).reshape((self.MAX_Ucar_num * 3 + self.MAX_Uav_num * 3,))
        self.obs[1] = np.append(mid, values=np.array(d).reshape(self.ed_num*2, ), axis=0)
        if self.nomal_obs == True:
            self.obs[0] = (self.obs[0]-self.obs[0].mean())/self.obs[0].var()
            self.obs[1] = (self.obs[1]-self.obs[1].mean())/self.obs[1].var()
        obs = {j:{"action_mask":self.comput_action_mask(j),"obs": self.obs[i]}
    ...
    def step(self, action_dict):
        ...
        mid = np.concatenate(([self.Uav["uav0"].state()], [self.Uav["uav1"].state()],
                                                     [ucar.state() for (_, ucar) in enumerate(self.Ucar)])).reshape(
            (self.MAX_Ucar_num * 3 + self.MAX_Uav_num*3,))
        d = [ed.state() for (_, ed) in enumerate(self.ED)]
        self.obs[0] = np.append(mid, values=np.array(d).reshape(self.ed_num*2, ), axis=0)

        mid = np.concatenate(([self.Uav["uav1"].state()],[self.Uav["uav0"].state()],
                                      [ucar.state() for (_, ucar) in enumerate(self.Ucar)])).reshape(
            (self.MAX_Ucar_num * 3 + self.MAX_Uav_num * 3,))
        d = [ed.state() for (_, ed) in enumerate(self.ED)]
        self.obs[1] = np.append(mid, values=np.array(d).reshape(self.ed_num*2, ), axis=0)
        if self.nomal_obs == True:
            self.obs[0] = (self.obs[0]-self.obs[0].mean())/self.obs[0].var()
            self.obs[1] = (self.obs[1]-self.obs[1].mean())/self.obs[1].var()
        return {j: {"action_mask": self.comput_action_mask(j), "obs": self.obs[i]}
             for i, j in enumerate(self._agent_ids)},{
            "uav0": reward,
            "uav1": reward,
